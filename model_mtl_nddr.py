...
class NDDRLayer(nn.Module, ABC):
    def __init__(self, out_channels):
        super().__init__()
        self.conv = nn.Conv3d(in_channels=2 * out_channels, out_channels=out_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.BN = nn.BatchNorm3d(out_channels)
...
class _CNN(nn.Module, ABC):
    def __init__(self, fil_num, drop_rate):
        ...
        self.nddr_1 = NDDRLayer(fil_num)
        self.nddr_2 = NDDRLayer(2 * fil_num)
        self.nddr_3 = NDDRLayer(2 * fil_num)
        self.nddr_4 = NDDRLayer(2 * fil_num)
        self.nddr_5 = NDDRLayer(2 * fil_num)
